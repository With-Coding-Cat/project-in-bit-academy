...
def DuplicateDB(duplicated_plate):
    ...
    duplicate_plate = db.All_collection.find_one({"car license plate":"{}".format(duplicated_plate)})    # 해당 번호판 정보가 데이터베이스에 있는지 조회
...
def Time_DuplicateDB(time_duplicated_plate, seconds):
    ...
    time_duplicate_data = db.All_collection.find_one({"car license plate":"{}".format(time_duplicated_plate), "time_raw" : {'$gte' : seconds - 10, '$lte':seconds}})
