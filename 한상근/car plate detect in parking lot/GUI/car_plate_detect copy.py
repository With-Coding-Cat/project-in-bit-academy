...
def detect(opt, OCR_model, OCR_pridiction_opt, ocr_device, converter, save_img=False):
    ...
    if not os.path.exists('parking_lot.db'):
        ...
        parking_db.execute('CREATE TABLE car_plate_data(id INTEGER, car_plate TEXT, detected_image_path_in TEXT, time_in TEXT, detected_image_path_out TEXT, time_out TEXT, time_in_sec TEXT, time_out_sec TEXT, price INTEGER)')
    ...
    db_io.execute("SELECT car_plate FROM car_plate_data WHERE detected_image_path_out == ' '")
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or view_img:  # Add bbox to image
                        ...
                        if predicted_plate_sting not in car_plate_in_parking_lot:
                            ...
                            os.makedirs(str(save_dir / 'img'), exist_ok=True)
                            image_path = str(save_dir / 'img/{}.jpg'.format(img_name_numbering))
                            cv2.imwrite(image_path, processed_image)
                            current_time_sec = time.time()
                            current_time = time.strftime('%Y년 %m월 %d일 %H시 %M분 %S초', time.localtime(current_time_sec))
                            data_to_insert = (img_name_numbering, predicted_plate_sting, image_path, current_time, current_time_sec)
                            ...
                            db_io.execute("INSERT INTO car_plate_data VALUES (?, ?, ?, ?, ' ', ' ', ?, ' ', 0)", data_to_insert)
