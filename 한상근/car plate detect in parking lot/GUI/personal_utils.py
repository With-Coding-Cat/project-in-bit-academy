...
def predict_plate(model, opt, image_to_give, device, converter):
    ...
    img_ori = cv2.resize(image_to_give, dsize=(200, 70), interpolation=cv2.INTER_CUBIC)
    ...
    dst = cv2.filter2D(blur, -1, sharpening_1)

    thresh = cv2.adaptiveThreshold(dst, 
        maxValue=255.0, 
        adaptiveMethod=cv2.ADAPTIVE_THRESH_GAUSSIAN_C, 
        thresholdType=cv2.THRESH_BINARY_INV, 
        blockSize=39, 
        C=9)
    rect_kern = cv2.getStructuringElement(cv2.MORPH_CROSS, (2,2))
    # apply dilation to make regions more clea
    ...
    for i, matched_chars in enumerate(matched_result):
        ...
        triangle_hypotenus = np.linalg.norm(
            np.array([sorted_chars[0]['cx'], sorted_chars[0
                                                         ]['cy']]) - 
        ...
        img_rotated = cv2.warpAffine(dilation, M=rotation_matrix, dsize=(width, height))
        rect_kern = cv2.getStructuringElement(cv2.MORPH_CROSS, (1,1))
        # apply dilation to make regions more clea
        img_dilation = cv2.dilate(img_rotated, rect_kern, iterations = 1)
    ...
    for i, plate_img in enumerate(plate_imgs):
        ...
        image_to_give = Image.fromarray(img_result)
    ...
    #print(image_to_give)
    #print(type(image_to_give))
    #print(image_to_give.shape)
